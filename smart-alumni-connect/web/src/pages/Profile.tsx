...
const Profile: React.FC<Props> = ({ alum, selfUser, onStartChat }) => {
    ...
    const [currentAlum, setCurrentAlum] = useState<any>(targetUser || {
        uid: 'placeholder',
    ...
    const handleEnhance = async () => {
        ...
        if (newBio) {
            setBio(newBio);
            // Auto-save the enhanced bio? Or let user save?
            // Let's auto-save for "magic" effect
            if (selfUser?.uid) {
                await updateUserProfile(selfUser.uid, { bio: newBio });
            }
        }
    ...
    const handleManualSave = async () => {
        if (isSelf && selfUser?.uid) {
            await updateUserProfile(selfUser.uid, { bio });
            alert("Bio updated!");
        }
    }
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-700 pb-24 md:pt-6">
                ...
                <div className="h-56 md:h-72 lg:h-80 rounded-[2.5rem] md:rounded-[4rem] bg-slate-950 overflow-hidden relative border border-white/10 shadow-xl z-0">
                    ...
                    <div className="absolute top-6 md:top-10 right-6 md:right-10 flex flex-col items-end gap-3 z-20">
                        ...
                        <div className="px-4 py-2 glass rounded-2xl text-[9px] md:text-[10px] font-bold text-white uppercase tracking-[0.2em] border border-white/20 shadow-lg whitespace-nowrap backdrop-blur-md">
                            📍 {currentAlum.location || "Global Node"}
                        </div>
                    ...
                    <div className="flex-1 pb-4 md:pb-12 text-center lg:text-left min-w-0">
                        <div className="flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-8 mb-4">
                            ...
                            <span className="inline-flex items-center self-center lg:self-auto px-5 py-2 rounded-2xl bg-slate-950 text-white text-[10px] md:text-[12px] font-black uppercase tracking-[0.3em] border border-white/10 shadow-xl">
                                Class of {currentAlum.graduationYear || 'XX'}
                            </span>
                        </div>
                    ...
                    <div className="pb-6 md:pb-14 flex flex-col items-center lg:items-end gap-3 md:gap-4 w-full md:w-auto">
                        ...
                        {isSelf && (
                            <button
                                ...
                                className="w-full md:w-auto px-8 md:px-14 py-4 md:py-6 bg-white border border-slate-200 text-slate-900 font-black rounded-3xl hover:bg-slate-50 transition-all shadow-sm active:scale-95 text-xs md:text-sm uppercase tracking-widest">
                                Save Changes
                            </button>
                        )}
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
                {[
                    { label: 'Mentorship Hours', value: '120+', icon: '🤝', color: 'from-blue-500/10 to-indigo-500/10', text: 'text-blue-600', sub: 'Elite Contributor' },
                    { label: 'Network Reach', value: '10+', icon: '💼', color: 'from-purple-500/10 to-pink-500/10', text: 'text-purple-600', sub: 'Verified Authority' },
                    { label: 'Network Pulse', value: currentAlum.vibePulse || 'Elite', icon: '⚡', color: 'from-amber-500/10 to-orange-500/10', text: 'text-orange-600', sub: 'High Tier Node' },
                ].map((stat, i) => (
